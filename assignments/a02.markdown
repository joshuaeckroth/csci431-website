...
The following diagram shows the operations that need to be understood by the git planner, and how the impact the workspace, index, and repository. See the sections below for details about each command.
...
You only need to define and submit the `domain.pddl` file. I will provide the various problem files that your domain needs to solve.
